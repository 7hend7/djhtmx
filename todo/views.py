...
class ListTodoView(ListView):
    ...
    def get_queryset(self):
        ...
        nsearch = self.request.GET.get("nsearch", None)
        print(f"===> get nsearch: {nsearch}")  

        queryset = super().get_queryset()
        queryset = queryset.filter(name__icontains=nsearch) if nsearch else queryset
